...
class MPERoIHead(RoIHeadTemplate):
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        #test
        #print(global_roi_grid_points.shape,roi_grid_radius.new_zeros(batch_size).int().fill_(roi_grid_radius.shape[1]).shape)
    ...
    def cluster_roi(self, batch_dict):
        batch_size = batch_dict['batch_size']
        batch_idx = batch_dict['point_coords'][:, 0]
        point_coords = batch_dict['point_coords'][:, 1:4]
        point_features = batch_dict['point_features']
        ...
        with torch.no_grad():
            ...
            pooled_empty_flag = rois.new_zeros(batch_size, num_rois)
            for bs_idx in range(batch_size):
                cur_points = batch_points[bs_idx] 
                cur_features = batch_point_features[bs_idx]
                ...
                for roi_box_idx in range(0, num_rois):
                    ...
                    if cur_roi_points.shape[0] >= num_sample:
                        ...
                        empty_num = num_sample - cur_roi_points.shape[0]
                        avg = torch.mean(cur_roi_points,0)
                        #print(avg.shape)
                        add_zeros = cur_roi_points.new_zeros(empty_num, 3)
                        add_zeros = avg.repeat(empty_num, 1)
                        add_zeros_features = cur_roi_features[0].repeat(empty_num, 1)
                        cur_roi_points_sample = torch.cat([cur_roi_points, add_zeros], dim=0)
                        cur_roi_features_sample = torch.cat([cur_roi_features, add_zeros_features], dim=0)
                    pooled_points[bs_idx, roi_box_idx, :, :] = cur_roi_points_sample  # (B,128,512,3)
                    pooled_features[bs_idx, roi_box_idx, :, :] = cur_roi_features_sample  # (B,128,512,130)
            pooled_features = torch.cat((pooled_points, pooled_features), dim=-1)#(B,128,512,133)
            #print('1:',pooled_features.shape)
            #clustering test  ###Undisclosed
            point_to_numpy = pooled_points[2:-1].detach().cpu().numpy()#(1, 128, 512, 3)
            #print(point_to_numpy.shape)
            #clustering = DBSCAN(eps=0.2,min_samples=2).fit(point_to_numpy)
            # canonical transformation
            roi_center = rois[:, :, 0:3]
            #device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
            device = roi_center.device
            point_center = np.mean(point_to_numpy,axis=0)
            point_center = torch.tensor(point_center).to(device)
            #print(point_center.shape)#[B, 128, 3]
            point_center = point_center[:,0,:]
            point_center = torch.cat((roi_center,point_center.unsqueeze(dim=0).expand(0,-1,-1)),dim=0)#(4,128,3)
            #print('2:',point_center.shape)
            #print('point_center:',point_center.shape)
            #print('roicenter:',roi_center.shape)
            #pooled_features[:, :, :, 0:3] -= roi_center.unsqueeze(dim=2)
            pooled_features[:, :, :, 0:3] -= point_center.unsqueeze(dim=2)
            pooled_features = pooled_features.view(-1, pooled_features.shape[-2],pooled_features.shape[-1])  # (B*64,512,3+130)
            pooled_features[:, :, 0:3] = common_utils.rotate_points_along_z(
            pooled_features[:, :, 0:3], -rois.view(-1, rois.shape[-1])[:, 6]
            )
            pooled_features[pooled_empty_flag.view(-1) > 0] = 0#(512,512,133)
            #sprint('3:',pooled_features.shape)
        return pooled_features
        #print('roi:',cur_roi_points)
    ...
    def forward(self, batch_dict):
        ...
        # print(hs.shape,2)
        #self.cluster_roi(batch_dict)
        pooled_features = self.cluster_roi(batch_dict)
        # RoI point pooling
        ...
        #print('merged_features_final:',merged_features)
        return batch_dict
...
class PyramidRoIHeadV2(RoIHeadTemplate):
    ...
    def roipool3d_gpu(self, batch_dict):
        ...
        point_features = batch_dict['point_features']
        rois = batch_dict['rois']  # (B, num_rois, 7 + C)
        batch_cnt = point_coords.new_zeros(batch_size).int()#用于计算每个批次中的点的数量
        for bs_idx in range(batch_size):
            batch_cnt[bs_idx] = (batch_idx == bs_idx).sum()

        assert batch_cnt.min() == batch_cnt.max()#用于确保每个批次中的点的数量相同

        #通过以下处理步骤，生成了批次级别的点云特征表示，其中batch_points存储了点的坐标信息，batch_point_features存储了点的特征信息
        point_scores = batch_dict['point_cls_scores'].detach()
        point_depths = point_coords.norm(dim=1) / self.model_cfg.ROI_POINT_POOL.DEPTH_NORMALIZER - 0.5#计算点的深度即兴趣点到原点的距离
        point_features_list = [point_scores[:, None], point_depths[:, None], point_features]
        point_features_all = torch.cat(point_features_list, dim=1)
        batch_points = point_coords.view(batch_size, -1, 3)
        batch_point_features = point_features_all.view(batch_size, -1, point_features_all.shape[-1])
        ...
        with torch.no_grad():
            ...
            batch_size = rois.shape[0]
            num_rois = rois.shape[1]
            num_sample = self.model_cfg.ROI_POINT_POOL.NUM_SAMPLED_POINTS#表示在ROI点池化中每个ROI中要采样的点的数量
            pooled_points = rois.new_zeros(batch_size, num_rois, num_sample, 3)
            pooled_features = rois.new_zeros(batch_size, num_rois, num_sample, 130)
            pooled_empty_flag = rois.new_zeros(batch_size, num_rois)#如果某个ROI为空，则对应位置的标志为1，否则为0
            for bs_idx in range(batch_size):
                cur_points = batch_points[bs_idx]  # (16384,3)当前批次中的点坐标
                cur_features = batch_point_features[bs_idx]  # (16384,130)
                cur_batch_boxes = rois[bs_idx]  # (64,7),[x,y,z,h,w,l,ry]当前批次中的ROI框坐标信息

                cur_radiis = torch.sqrt((cur_batch_boxes[:, 4] / 2) ** 2 + (cur_batch_boxes[:, 5] / 2) ** 2) * 1.2#每个ROI框的半径，增加了一定的扩展范围
                #dis计算了每个点与ROI框中心点的欧氏距离，形状为(16384, 64)。其中，dis[i, j]表示第i个点与第j个ROI框中心点的欧氏距离
                dis = torch.norm((cur_points[:, :2].unsqueeze(0) - cur_batch_boxes[:, :2].unsqueeze(1).repeat(1,cur_points.shape[0],1)),dim=2)
                point_mask = (dis <= cur_radiis.unsqueeze(-1))#布尔掩码，用于表示每个点是否在相应的ROI框内
                #对ROI框内的点进行采样
                for roi_box_idx in range(0, num_rois):
                    cur_roi_points = cur_points[point_mask[roi_box_idx]]
                    cur_roi_features = cur_features[point_mask[roi_box_idx]]
     #如果ROI框内的点数量大于等于num_sample，则从中随机选择num_sample个点作为采样结果
                    if cur_roi_points.shape[0] >= num_sample:
                        random.seed(0)
                        index = np.random.randint(cur_roi_points.shape[0], size=num_sample)
                        cur_roi_points_sample = cur_roi_points[index]
                        cur_roi_features_sample = cur_roi_features[index]
    #如果ROI框内没有点，则将cur_roi_points_sample和cur_roi_features_sample设置为全零，并将对应的pooled_empty_flag标记为roi_box_idx
                    elif cur_roi_points.shape[0] == 0:
                        cur_roi_points_sample = cur_roi_points.new_zeros(num_sample, 3)
                        cur_roi_features_sample = cur_roi_features.new_zeros(num_sample, 130)
                        pooled_empty_flag[bs_idx, roi_box_idx] = roi_box_idx
    #如果ROI框内的点数量不足num_sample，则需要进行填充。首先计算需要填充的点的数量，然后使用重复的方式从ROI框内的第一个点进行填充，以达到num_sample个点的数量
                    else:
                        empty_num = num_sample - cur_roi_points.shape[0]
                        add_zeros = cur_roi_points.new_zeros(empty_num, 3)
                        add_zeros = cur_roi_points[0].repeat(empty_num, 1)
            ...
            # canonical transformation
            roi_center = rois[:, :, 0:3]
            pooled_features[:, :, :, 0:3] -= roi_center.unsqueeze(dim=2)#得到了每个采样点相对于ROI框中心点的相对坐标
    ...
    def forward(self, batch_dict):
        ...
        # RoI point pooling
        #(B*128,512,128+5)其中B表示批次大小，128表示ROI的数量，512表示每个ROI中采样的点的数量，128+5表示每个点的特征维度为128维，加上5维的坐标和大小信息
        pooled_features = self.roipool3d_gpu(batch_dict)  #(total_rois,num_sampled_points,3+C)(B*128,512,128+5)
        #batch_size_rcnn = pooled_features.shape[0]
        #pooled_features = pooled_features.reshape(batch_size_rcnn,-1,1) #(Bx128,512*133,1)
        xyz_input = pooled_features[..., 0:5].transpose(1, 2).unsqueeze(dim=3).contiguous()#(B*128, 5, 512, 1)
        #将正常获取的特征和通过点送到xyz卷积得出的特征进行融合，特征增强
        xyz_features = self.xyz_up_layer(xyz_input)
        point_features = pooled_features[..., 5:].transpose(1, 2).unsqueeze(dim=3)#(B*128, 128, 512, 1)
        merged_features = torch.cat((xyz_features, point_features), dim=1)#(B*128, 128, 512, 1)
        merged_features = self.merge_down_layer(merged_features)
        batch_size_rcnn = merged_features.shape[0]
        merged_features = merged_features.reshape(batch_size_rcnn,-1,1) #(Bx128,512*128,1)
...
class PyramidRoIHeadV3(RoIHeadTemplate):
