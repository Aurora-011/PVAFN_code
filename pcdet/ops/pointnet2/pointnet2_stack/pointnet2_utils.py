...
class QueryAndGroupPyramidAttentionV2(nn.Module): 
    def __init__(self, nsample):
        super().__init__()
        self.nsample = nsample

    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_r, new_xyz_batch_cnt, features,features2):
        assert xyz.shape[0] == xyz_batch_cnt.sum(), 'xyz: %s, xyz_batch_cnt: %s' % (str(xyz.shape), str(new_xyz_batch_cnt))
        assert new_xyz.shape[0] == new_xyz_batch_cnt.sum(), \
            'new_xyz: %s, new_xyz_batch_cnt: %s' % (str(new_xyz.shape), str(new_xyz_batch_cnt))
        ...
        grouped_features2 = grouping_operation(features2, xyz_batch_cnt, idx, new_xyz_batch_cnt)  # (M1 + M2, C, nsample)
        grouped_features2[empty_ball_mask] = 0
        grouped_features2 = torch.cat([grouped_xyz, grouped_features2], dim=1)  # (M1 + M2 ..., C + 3, nsample)

        return grouped_xyz.contiguous(), grouped_features.contiguous(),grouped_features2.contiguous(), empty_ball_mask
